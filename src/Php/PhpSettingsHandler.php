...
class PhpSettingsHandler
{
     ...
     * @param LoggerInterface $logger Activates status message output to a PSR-3 logger
     *
    ...
    {
        // MODIFIED: function made final + logger
    ...
    private function writeTmpIni(array $iniFiles)
    {
        ...
        file_put_contents('tmp-php.ini', $content);
    ...
    private function getCommand(array $args)
    {
        ...
        // MODIFIED
        $this->notify(Status::INFO, "Restart with: \"$cmd\"");
        // --MODIFIED
    ...
    {
        $this->statusWriter = new Status($logger, $this->restart, $this->envAllowIniSettings);
    }
    ...
    public function check()
    {
        ...
        $envArgs = explode('|', strval(getenv($this->envAllowIniSettings)), 4);
        ...
        if (self::RESTART_ID === $envArgs[0] && count($envArgs) >= 3) {
            ...
            putenv($this->envAllowIniSettings);
            $version = $envArgs[1];
    ...
    private function setEnvironment($scannedInis, $scanDir, array $iniFiles)
    {
        ...
        return putenv($this->envAllowIniSettings.'='.implode('|', $envArgs));
    }
